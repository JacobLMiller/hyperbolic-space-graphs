...
class HMDS_step:
    ...
    def solve(self,num_iter=1000,epsilon=1e-3,debug=False,step_type='default'):
        ...
        while count < num_iter:
            ...
            if step_type == "default":
                ...
                step = self.compute_step_size_frac(count,num_iter,step)
            elif step_type =="sqrt":
                step = self.compute_step_size_sqrt(count,num_iter,step)
            ...
            if debug and count < 30:
                ...
                print(stress)
                loss.append(stress)
            if debug and count > 31 and count % 100 == 0:
                stress = self.calc_stress()
                print(stress)
                loss.append(stress)
    ...
    def compute_step_size(self,count,num_iter):
        lamb = math.log(self.eta_min/self.eta_max)/(15-1)
        return self.eta_max*math.exp(lamb*count)

    def compute_step_size_frac(self,count,num_iter,step):
        a = 1/self.w_max
        b = -math.log(self.eta_min/self.eta_max)/(15-1)
        return a/(1+b*count)

    def compute_step_size_sqrt(self,count,num_iter,step):
        a = 1/self.w_max
        b = -math.log(self.eta_min/self.eta_max)/(15-1)
