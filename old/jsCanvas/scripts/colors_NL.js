...
let random_circle_coordinates = function() {
  ...
  HyperbolicCanvas.scripts['colors_NL'] = function (canvas) {
    ...
    for (i in t.children){
      ...
      if(t.children[i].type == 'node_stmt'){
        ...
        for(j in t.children[i].attr_list){
          ...
          if(t.children[i].attr_list[j].id === 'label'){
            V[v].label = t.children[i].attr_list[j].eq
          }
          if(t.children[i].attr_list[j].id === "color"){
            V[v].color = t.children[i].attr_list[j].eq
          }
        }
        V[v].neighbors = [];
      }
    ...
    var render = function (event) {
      ...
      for(i in G.nodeList){
        ...
        if( nodeDis < 1){
          fontSize = Math.ceil(15 *(1-nodeDis));
