...
let bigCount = 0;
let keepGoing = true;
let graphStr = "graphs/triangle_thing.dot"
...
var generate_N_polygon = function(n){
    ...
    R = 1
    for (i = 0; i<n; i++){
      verticesList.push([R+Math.sin(i*theta)-.5,R+math.cos(i*theta)-.5]);
    }
    return verticesList
...
var KK_Step = function(nodes,destinationList,current_max_delta){
  ...
  for (i in destinationList){
    coord = [destinationList[i].getX(),destinationList[i].getY()]
    }
}
...
let random_circle_coordinates = function() {
  ...
  HyperbolicCanvas.scripts['compute-and-display'] = function (canvas) {
    ...
    totalCount += 1
    /*while (myCount < ITERATIONS && keepGoing) {
    ...
    var render = function (event) {
      ...
      A = HyperbolicCanvas.Point.givenCoordinates(0,0);
      Z = HyperbolicCanvas.Point.givenCoordinates(.2,-.54);

      path = canvas.pathForHyperbolic(HyperbolicCanvas.Line.givenTwoPoints(A,Z));
      canvas.stroke(path);

      path = canvas.pathForHyperbolic(HyperbolicCanvas.Circle.givenHyperbolicCenterRadius(A,.1));
      canvas.fillAndStroke(path);

      path = canvas.pathForHyperbolic(HyperbolicCanvas.Circle.givenHyperbolicCenterRadius(Z,.1));
      canvas.fillAndStroke(path);

      if(keepGoing){
        distance = 2*Math.atanh(Z.getEuclideanRadius());
        console.log(distance)
        norm = Z.getEuclideanRadius();
        myComplex = math.Complex(Z.getX(),Z.getY());
        normalized = math.divide(myComplex,norm);
        console.log(normalized)
        console.log(normalized.toPolar())
        console.log(math.multiply(normalized,distance))
        console.log(math.multiply(normalized,distance).toPolar())
        console.log(math.Complex.fromPolar(distance,Z.getAngle()));
        keepGoing = false;
      }
